...
def batch_norm_relu(inputs, is_training, relu=True, init_zero=False,
  ...
  """
  if init_zero:
    gamma_initializer = tf1.zeros_initializer()
  else:
    gamma_initializer = tf1.ones_initializer()
  ...
  inputs = tf1.layers.batch_normalization(
    inputs=inputs,
    axis=axis,
...
def dropblock(net, is_training, keep_prob, dropblock_size,
  ...
  tf1.logging.info('Applying DropBlock: dropblock_size {}, net.shape {}'.format(
  ...
  w_i, h_i = tf1.meshgrid(tf1.range(width), tf1.range(width))
  valid_block_center = tf1.logical_and(
      tf1.logical_and(w_i >= int(dropblock_size // 2),
                     w_i < width - (dropblock_size - 1) // 2),
      tf1.logical_and(h_i >= int(dropblock_size // 2),
                     h_i < width - (dropblock_size - 1) // 2))

  valid_block_center = tf1.expand_dims(valid_block_center, 0)
  valid_block_center = tf1.expand_dims(
      valid_block_center, -1 if data_format == 'channels_last' else 0)

  randnoise = tf1.random_uniform(net.shape, dtype=tf1.float32)
  block_pattern = (1 - tf1.cast(valid_block_center, dtype=tf1.float32) + tf1.cast(
      (1 - seed_drop_rate), dtype=tf1.float32) + randnoise) >= 1
  block_pattern = tf1.cast(block_pattern, dtype=tf1.float32)

  if dropblock_size == width:
    block_pattern = tf1.reduce_min(
        block_pattern,
    ...
    block_pattern = -tf1.nn.max_pool(
        -block_pattern, ksize=ksize, strides=[1, 1, 1, 1], padding='SAME',
        data_format='NHWC' if data_format == 'channels_last' else 'NCHW')

  percent_ones = tf1.cast(tf1.reduce_sum((block_pattern)), tf1.float32) / tf1.cast(
      tf1.size(block_pattern), tf1.float32)

  net = net / tf1.cast(percent_ones, net.dtype) * tf1.cast(
...
def fixed_padding(inputs, kernel_size, data_format='channels_last'):
  ...
  if data_format == 'channels_first':
    padded_inputs = tf1.pad(inputs, [[0, 0], [0, 0],
                                    [pad_beg, pad_end], [pad_beg, pad_end]])
  else:
    padded_inputs = tf1.pad(inputs, [[0, 0], [pad_beg, pad_end],
                                    [pad_beg, pad_end], [0, 0]])
...
def sk_conv2d(inputs, filters, strides, sk_ratio, min_dim=32,
  ...
  inputs = tf1.stack(tf1.split(inputs, num_or_size_splits=2, axis=channel_axis))

  # Mixing weights for two streams.
  mid_dim = max(int(filters * sk_ratio), min_dim)
  global_features = tf1.reduce_mean(
      tf1.reduce_sum(inputs, axis=0), pooling_axes, keepdims=True)
  global_features = tf1.layers.conv2d(
  ...
  mixing = tf1.layers.conv2d(
      inputs=global_features, filters=2 * filters, kernel_size=1, strides=1,
      kernel_initializer=tf1.variance_scaling_initializer(),
      use_bias=False, data_format=data_format)
  mixing = tf1.stack(tf1.split(mixing, num_or_size_splits=2, axis=channel_axis))
  mixing = tf1.nn.softmax(mixing, axis=0)

  return tf1.reduce_sum(inputs * mixing, axis=0)
...
def se_layer(inputs, filters, se_ratio, data_format='channels_last'):
  ...
  se_tensor = tf1.reduce_mean(
      inputs, spatial_dims, keepdims=True)
  se_tensor = se_expand(tf1.nn.relu(se_reduce(se_tensor)))
  return tf1.sigmoid(se_tensor) * inputs
...
def residual_block(inputs, filters, is_training, strides,
  ...
  if use_projection:
    ...
    if FLAGS.sk_ratio > 0:  # Use ResNet-D (https://arxiv.org/abs/1812.01187)
      ...
      inputs = tf1.layers.average_pooling2d(
  ...
  if use_projection:
    ...
    if FLAGS.sk_ratio > 0:  # Use ResNet-D (https://arxiv.org/abs/1812.01187)
      if strides > 1:
        ...
        shortcut = inputs
      shortcut = tf1.layers.average_pooling2d(
...
def block_group(inputs, filters, block_fn, blocks, strides, is_training, name,
  ...
  return tf1.identity(inputs, name)
...
def resnet_v1_generator(block_fn, layers, width_multiplier,
  ...
  def model(inputs, is_training):
    ...
    if cifar_stem:
      ...
      inputs = tf1.identity(inputs, 'initial_conv')
      inputs = batch_norm_relu(inputs, is_training, data_format=data_format)
      inputs = tf1.identity(inputs, 'initial_max_pool')
    else:
      if FLAGS.sk_ratio > 0:  # Use ResNet-D (https://arxiv.org/abs/1812.01187)
        inputs = conv2d_fixed_padding(
      ...
      inputs = tf1.layers.max_pooling2d(
    ...
    def filter_trainable_variables(trainable_variables, after_block):
      ...
      if after_block == 0:
        trainable_variables[after_block] = tf1.trainable_variables()
      else:
    ...
    def add_to_collection(trainable_variables, prefix):
      ...
      for after_block, variables in trainable_variables.items():
        ...
        if len(tf1.get_collection(collection)) == 0:
          for var in variables:
            tf1.add_to_collection(collection, var)
    ...
    if FLAGS.train_mode == 'finetune' and FLAGS.fine_tune_after_block == 0:
      inputs = tf1.stop_gradient(inputs)
    ...
    inputs = tf1.identity(inputs, 'final_feature_map')
    if data_format == 'channels_last':
      inputs = tf1.reduce_mean(inputs, [1, 2])
    else:
      inputs = tf1.reduce_mean(inputs, [2, 3])
    inputs = tf1.identity(inputs, 'final_avg_pool')
